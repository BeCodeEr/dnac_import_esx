...
if not os.path.isfile("antennaTypes.json"):
...
for i in list_esx:
  ...
  "simulatedRadios": ["""
    ...
    #Delete ',' (in Line 146 of this script) in the end for the last entry
    ...
    #search for APs in Floors
    with open('./' + myExtractfolder + '/accessPoints.json') as json_file_aps:
      ap_data = json.load(json_file_aps)    

    for a in floor_data['floorPlans']:
      floor_name = a['name']
      floor_id = a['id']
      DNA_Structure_File.write('\t\tFloor: ' + floor_name + '\n')
      for b in ap_data['accessPoints']: 
        # If 'location' for the AP is present, then the AP is on a floor
        if "location" in b:
          res_apname = b['name']
          res_floorID = b['location']['floorPlanId']
          if floor_id == res_floorID:
            DNA_Structure_File.write('\t\t\tAccesspoint: ' + res_apname  + '\n')
    DNA_Structure_File.write('\n=============================================\n')    
